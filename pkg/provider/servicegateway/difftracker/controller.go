...
var (
	initializeControllerDiffTracker = InitializeFromCluster
	startControllerPodInformer      = func(ctx context.Context, tracker *DiffTracker) error {
		return tracker.SetUpPodInformer(ctx.Done())
	}
)

// Controller owns the ServiceGateway runtime and implements cloudprovider.LoadBalancer.
...
// NewController creates an unstarted ServiceGateway controller.
...
func (c *Controller) Start(
	...
	if c.tracker != nil {
		return fmt.Errorf("ServiceGateway controller is already started")
	}
	if informerFactory == nil {
		return fmt.Errorf("ServiceGateway controller requires a shared informer factory")
	}
	if kubeClient == nil {
		return fmt.Errorf("ServiceGateway controller requires an initialized Kubernetes client")
	}
	if eventRecorder == nil {
		return fmt.Errorf("ServiceGateway controller requires an initialized event recorder")
	}
	...
	tracker, err := initializeControllerDiffTracker(runtimeCtx, c.config, clientFactory, kubeClient)
	if err != nil {
	...
	if err := startControllerPodInformer(runtimeCtx, tracker); err != nil {
		return fmt.Errorf("start filtered Pod informer: %w", err)
	}

	c.tracker = tracker
	RecordServiceGatewayEnabled()
	return nil
}

// RegisterInformers installs the shared Node and EndpointSlice handlers owned by ServiceGateway.
func RegisterInformers(informerFactory informers.SharedInformerFactory, tracker *DiffTracker) (func(), error) {
	removeHandler := func(informer cache.SharedIndexInformer, registration cache.ResourceEventHandlerRegistration, name string) {
		if err := informer.RemoveEventHandler(registration); err != nil {
			klog.Errorf("Failed to unregister ServiceGateway %s handlers: %v", name, err)
		}
	}
	...
	nodeRegistration, err := nodeInformer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			node, ok := obj.(*v1.Node)
			if !ok {
				klog.Errorf("ServiceGateway node add received unexpected object: %T", obj)
				return
			}
		...
		UpdateFunc: func(oldObj, newObj interface{}) {
			...
			if !oldOK || !newOK {
				klog.Errorf("ServiceGateway node update received unexpected objects: %T, %T", oldObj, newObj)
				return
			}
		...
		DeleteFunc: func(obj interface{}) {
			...
			if err != nil {
				klog.Errorf("Cannot process ServiceGateway Node deletion: %v", err)
				return
			}
	...
	endpointSliceRegistration, err := endpointSliceInformer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			if !ok {
				klog.Errorf("ServiceGateway EndpointSlice add received unexpected object: %T", obj)
				return
			}
		...
		UpdateFunc: func(oldObj, newObj interface{}) {
			...
			if !oldOK || !newOK {
				klog.Errorf("ServiceGateway EndpointSlice update received unexpected objects: %T, %T", oldObj, newObj)
				return
			}
		...
		DeleteFunc: func(obj interface{}) {
			...
			if err != nil {
				klog.Errorf("Cannot process ServiceGateway EndpointSlice deletion: %v", err)
				return
			}
