...
func (dt *DiffTracker) SetUpPodInformer(stopCh <-chan struct{}) error {
	if stopCh == nil {
		return fmt.Errorf("setUpPodInformerForEgress: stop channel must not be nil")
	}
	...
	if !cache.WaitForNamedCacheSync("servicegateway-egress-pods", stopCh, podInformer.HasSynced) {
