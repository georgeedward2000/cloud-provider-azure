...
// reconcileServiceGatewayNodeIPChange replays the cached EndpointSlices hosting a pod on nodeName
// into the difftracker when the node's InternalIP set changes, or the node is added or removed.
// The EndpointSlice informer never fires for a node-only change (the slice content is unchanged),
// and it derives an endpoint's "old" location from the live (already-updated) node cache, so it can
// never emit the removal of a pod from its previous node IP. oldNodeIPs/newNodeIPs are taken from
// the informer's node objects rather than the mutable cache, so the old location is accurate: each
// affected pod is moved from its old same-family location to its new one. Empty newNodeIPs (node
// deleted) drains the pods; empty oldNodeIPs (node added) registers a pod dropped while its node was
// not yet cached. Egress pods are unaffected — they resolve their node location from pod.Status.HostIPs.
func (az *Cloud) reconcileServiceGatewayNodeIPChange(nodeName string, oldNodeIPs, newNodeIPs []string) {
	if !az.ServiceGatewayEnabled || az.diffTracker == nil || nodeName == "" {
		return
	}
	...
	az.endpointSlicesCache.Range(func(_, value interface{}) bool {
		...
		ipv6 := es.AddressType == discovery_v1.AddressTypeIPv6
		oldLocation, oldOK := difftracker.SelectSameFamilyNodeIP(oldNodeIPs, ipv6)
		newLocation, newOK := difftracker.SelectSameFamilyNodeIP(newNodeIPs, ipv6)
		if !oldOK && !newOK {
			return true
		}
	...
	for serviceUID, delta := range perService {
		...
		klog.V(2).Infof("reconcileServiceGatewayNodeIPChange: node %s changed, moving pod addresses for service %s (old=%d new=%d)",
			nodeName, serviceUID, len(delta.oldAddresses), len(delta.newAddresses))
		az.diffTracker.UpdateEndpoints(serviceUID, delta.oldAddresses, delta.newAddresses)
	}
}
